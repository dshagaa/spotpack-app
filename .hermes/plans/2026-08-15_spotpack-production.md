...
|--------|------|------|-------------|---------|
| GET | `/api/events` | none | Read `events/index.json` | `{ events: [...] }` |
| GET | `/api/events/{id}` | none | Read `events/{id}.json` | `{ event, items: [...] }` |
| POST | `/api/events` | general+ | Write `events/{id}.json` + update `index.json` | `{ event }` |
| PATCH | `/api/events/{id}` | general+ | Read → modify → write `events/{id}.json` + update `index.json` | `{ event }` |
| DELETE | `/api/events/{id}` | maintainer | Delete `events/{id}.json` + update `index.json` | `{ deleted: true }` |
| POST | `/api/import` | general+ | Upload image to R2 → MiMo → write `events/{id}.json` + update `index.json` | `{ success, count, items }` |
| POST | `/api/keys` | maintainer | Read `auth/keys.json` → append → write | `{ key_hash, role, label }` |
